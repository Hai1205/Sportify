...
class FollowUserView(GenericAPIView):
    def post(self, request, userId, opponentId):
        try:
    ...
    def put(self, request, userId):
        try:
            ...
            if avatar := request.FILES.get("avatar"):
                ...
                user.avatarUrl = cloud_service.upload_file(avatar)
...
class DeleteUserView(GenericAPIView):
    ...
    def delete(self, request, userId):
        try:
            ...
            cloud_service = CloudinaryService()
            cloud_service.delete_file(avatarUrl)
            ...
            for album in albums:
                cloud_service.delete_file(album.thumbnailUrl)
                album.delete()
...
class RequireUpdateUserToArtistView(GenericAPIView):
    ...
    def post(self, request, userId):
        try:
            ...
            s3_service = CloudinaryService()
            song1AudioUrl = s3_service.upload_file(song1Audio)
            song2AudioUrl = s3_service.upload_file(song2Audio)
            song3AudioUrl = s3_service.upload_file(song3Audio)
...
class DeleteArtistApplicationView(GenericAPIView):
    ...
    def delete(self, request, applicationId):
        try:
            ...
            for song in songs:
                s3_service.delete_file(song.audioUrl)
                song.delete()
...
class UserListView(GenericAPIView):
    ...
    get = GetAllUserView.get
    post = CreateUserView.post


class UserDetailView(GenericAPIView):
    """GET = user detail, PUT = update, DELETE = delete"""
    def get_permissions(self):
    ...
    get = GetUserView.get
    put = UpdateUserView.put
    delete = DeleteUserView.delete


class UserArtistApplicationView(GenericAPIView):
    """GET = get user's artist application, POST = submit artist application"""
    get = GetArtistApplication.get
    post = RequireUpdateUserToArtistView.post
    get_audio_duration = RequireUpdateUserToArtistView.get_audio_duration


class ArtistApplicationDetailView(GenericAPIView):
    """PUT = respond to application, DELETE = delete application"""
    permission_classes = [IsAdminUser]

    put = ResponseUpdateUserToArtistView.put
    delete = DeleteArtistApplicationView.delete