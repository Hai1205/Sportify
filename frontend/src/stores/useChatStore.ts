...
export const useChatStore = create<ChatStore>((set, get) => ({
    ...
    getMessages: async (conversationId, page = 1, reset = false) => {
        ...
        try {
            ...
            const fetchedMessages = (response.data?.results.messages || []).map((msg: any) => ({
                ...
                roomId: String(msg.conversation),
            }));
    ...
    initSocket: (userId, roomId = null) => {
        ...
        let wsBaseUrl = import.meta.env.VITE_SERVER_WS_URL || `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}`;
            ...
            try {
                ...
                if (data.type === "users_online") {
                    ...
                    const resolvedSenderId = String(
                        message.senderId || message.sender_id || message.sender_data?.id || message.sender || ''
                    );
                    ...
                    const processedMessage: Message = {
                        ...
                        roomId: String(message.roomId || message.room_id || conversation),
                        createdAt: message.createdAt || message.created_at || new Date().toISOString(),
                        updatedAt: message.updatedAt || message.updated_at || new Date().toISOString(),
                    };

                    const activeRoomId = get().activeRoomId;
                    if (processedMessage.roomId === activeRoomId || conversation === activeRoomId) {
                        set(state => {
                            // Check if this is an echo of a pending message we sent
                            // (pending messages only exist for our own outgoing messages)
                            const pendingIndex = state.messages.findIndex(msg =>
                                msg.pending && msg.content === processedMessage.content
                            );
                            if (pendingIndex !== -1) {
                                const updated = [...state.messages];
                                updated[pendingIndex] = { ...processedMessage, pending: false };
                                return { messages: updated };
                            }

                            // Skip if already exists by id
                            if (state.messages.some(msg => msg.id === processedMessage.id)) {
                                return {};
                            }
                    ...
                    set(state => {
                        const messageRoomId = processedMessage.roomId || conversation;
